...
def match_crawl(sy):
    ...
    cnt = start = datetime.date(sy, start[37:39], start[47:49])#시작일 선언 cnt는 계속 변하는 수고 혹시 시작일이 필요 할 수 있으므로 start를 같이 선언해준다

    while cnt < datetime.date(sy, end[37:39], end[47:49]) + datetime.timedelta(days = 1):#마지막날에서 1일을 더 더한 수 까지
        month = cnt[5:7] #이번 날짜의 달
        day = cnt[8:10] #이번 날짜의 일
        url = base_url + "&sm=" + str(month) + "&sd=" + str(day) #나중에 달과 일을 또 안쓰면 그냥 직접 넣는걸로 코드를 바꾸자
        source_code = requests.get(url)
        plain_text = source_code.text
        soup = BeautifulSooup(plain_text, 'html.parser')
            ...
            for link in get_href.find_all('a'):
                ...
                while tag.next.string != "\n\:
                    ...
                    if string.count(" ")==2:
                        ...
                        home_score = int(string[nbsp_index+1:colon_index])
                        score_diff = home_score-away_score
                    

                    if match_win>0 : #홈이 이겼으면, 다음 스테이트를 이긴거에 더하
